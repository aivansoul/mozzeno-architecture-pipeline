...
if(!window.matchMedia('(prefers-reduced-motion: reduce)').matches){document.body.classList.add('motion-ready');revealObserver=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add('is-visible');revealObserver.unobserve(entry.target)}})},{threshold:.08,rootMargin:'0px 0px -28px'});document.querySelectorAll('.section-head,.metrics article,.pipeline,.detail-panel,.tool-card,.plane-card,.roadmap-row,.proof-level,.callout,.do-dont article').forEach((element,index)=>{element.classList.add('reveal');element.style.setProperty('--reveal-order',String(index%6));revealObserver.observe(element)})}
